...
def build_long_rsi50_trend_hold_prediction(
    ...
    prev_rsi9_for_cycle = status.rsi50_cycle_last_rsi9 if status is not None else None
    ...
    if status is not None and use_first_cross_filter:
        if rsi_now <= exit_rsi9_max:
            ...
            if crossed_from_below or morning_initial_eligible:
                ...
                storage and storage.log_structured("INFO", event_type, {
                    "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now,
                    "prev_rsi9": prev_rsi9_for_cycle, "entry_rsi9_min": entry_rsi9_min, "exit_rsi9_max": exit_rsi9_max,
                ...
                storage and storage.log_structured("INFO", "LONG_RSI50_CYCLE_ENTRY_ELIGIBLE", {
                    "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now,
                    "prev_rsi9": prev_rsi9_for_cycle, "entry_rsi9_min": entry_rsi9_min, "exit_rsi9_max": exit_rsi9_max,
                    "ema13": ema13_now, "ema13_lookback_value": ema13_lookback_value, "ema13_delta_ticks": ema13_delta_ticks,
                    "reference_close": bar1.close, "best_ask": best_ask, "entry_deviation_ticks": entry_deviation_ticks,
        ...
        if rsi_now >= entry_rsi9_min and not cycle_allows_entry:
            ...
            storage and storage.log_structured("INFO", "LONG_RSI50_ENTRY_BLOCKED_BY_RSI50_CYCLE_ALREADY_USED", {
                "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now,
                "prev_rsi9": prev_rsi9_for_cycle, "entry_rsi9_min": entry_rsi9_min, "exit_rsi9_max": exit_rsi9_max,
                "ema13": ema13_now, "ema13_lookback_value": ema13_lookback_value, "ema13_delta_ticks": ema13_delta_ticks,
                "reference_close": bar1.close, "best_ask": best_ask, "entry_deviation_ticks": entry_deviation_ticks,
    ...
    if status is not None:
        status.rsi50_cycle_last_rsi9 = rsi_now
...
def build_long_rsi35_rebound_prediction(
    ...
    if len(closes) <= RSI9_PERIOD + 1:
        return None
    rsi_now = rsi9_wilder(closes, RSI9_PERIOD)
    rsi_prev = status.rsi35_rebound_last_rsi9
    if rsi_now is None:
        return None
    ...
    if rsi_now <= entry_watch_max and not status.rsi35_rebound_cycle_consumed:
        if not status.rsi35_rebound_watch_active:
            ...
            if storage:
                storage.log_structured("INFO", "LONG_RSI35_REBOUND_WATCH_STARTED", {
    ...
    if storage and status.rsi35_rebound_watch_active:
        storage.log_structured("INFO", "LONG_RSI35_REBOUND_ENTRY_EVALUATED", {
            "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": rsi_prev,
            "close": bar1.close, "best_ask": best_ask, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None,
            "reference_close": bar1.close, "entry_deviation_ticks": entry_deviation_ticks, "exit_deviation_ticks": None,
            "entry_watch_rsi9_max": entry_watch_max, "exit_watch_rsi9_min": exit_watch_min,
    ...
    status.rsi35_rebound_last_rsi9 = rsi_now
    return PredictionSnapshot(
        ts=bar1.ts,
...
def should_exit(
    ...
    if pos.strategy == LONG_RSI35_STRATEGY:
            ...
            if storage is not None:
                storage.log_structured(
                    ...
                    "LONG_RSI35_REBOUND_HARD_STOP_TRIGGERED",
                    {
                        ...
                        "rsi9": pred.rsi9_value,
                        "prev_rsi9": status.rsi35_rebound_last_rsi9 if status is not None else None,
        ...
        rsi_now = pred.rsi9_value
        prev_rsi = status.rsi35_rebound_last_rsi9 if status is not None else None
        if bar1_new is not None and rsi_now is not None:
            if status is not None and rsi_now >= exit_watch_rsi9_min and not status.rsi35_rebound_exit_watch_active:
                ...
                if storage is not None:
                    storage.log_structured("INFO", "LONG_RSI35_REBOUND_EXIT_WATCH_STARTED", {"ts": f.ts.isoformat(), "bar_ts": bar1_new.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi, "close": bar1_new.close, "best_ask": latest_snapshot.sell1_price if latest_snapshot is not None else None, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None, "reference_close": bar1_new.close, "entry_deviation_ticks": None, "exit_deviation_ticks": None, "entry_watch_rsi9_max": float(cfg35.get("entry_watch_rsi9_max", 35)), "exit_watch_rsi9_min": exit_watch_rsi9_min, "position_entry_price": pos.entry_price, "unrealized_ticks": pnl_ticks, "cycle_state": {"exit_watch_active": True}, "block_reason": "", "signal": "EXIT_WATCH", "can_enter": None})
            ...
            if storage is not None and status is not None and status.rsi35_rebound_exit_watch_active:
                storage.log_structured("INFO", "LONG_RSI35_REBOUND_EXIT_EVALUATED", {"ts": f.ts.isoformat(), "bar_ts": bar1_new.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi, "close": bar1_new.close, "best_ask": latest_snapshot.sell1_price if latest_snapshot is not None else None, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None, "reference_close": bar1_new.close, "entry_deviation_ticks": None, "exit_deviation_ticks": reference_close_deviation_ticks(latest_snapshot.buy1_price if latest_snapshot is not None else None, bar1_new.close), "entry_watch_rsi9_max": float(cfg35.get("entry_watch_rsi9_max", 35)), "exit_watch_rsi9_min": exit_watch_rsi9_min, "position_entry_price": pos.entry_price, "unrealized_ticks": pnl_ticks, "cycle_state": {"exit_watch_active": status.rsi35_rebound_exit_watch_active}, "block_reason": "" if exit_triggered else "RSI_NOT_TURNED_DOWN_AFTER_50", "signal": "EXIT" if exit_triggered else "HOLD", "can_enter": None})
            if exit_triggered:
                if storage is not None:
                    storage.log_structured("INFO", "LONG_RSI35_REBOUND_EXIT_TRIGGERED", {"ts": f.ts.isoformat(), "bar_ts": bar1_new.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi, "close": bar1_new.close, "best_ask": latest_snapshot.sell1_price if latest_snapshot is not None else None, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None, "reference_close": bar1_new.close, "entry_deviation_ticks": None, "exit_deviation_ticks": reference_close_deviation_ticks(latest_snapshot.buy1_price if latest_snapshot is not None else None, bar1_new.close), "entry_watch_rsi9_max": float(cfg35.get("entry_watch_rsi9_max", 35)), "exit_watch_rsi9_min": exit_watch_rsi9_min, "position_entry_price": pos.entry_price, "unrealized_ticks": pnl_ticks, "cycle_state": {"exit_watch_active": True}, "block_reason": "", "signal": "EXIT", "can_enter": None, "exit_reason": LONG_RSI35_EXIT_REASON})
                if status is not None:
                    status.rsi35_rebound_last_rsi9 = rsi_now
