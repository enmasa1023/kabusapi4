...
def log_structured_once_per_bar(
    ...
    if status is None or bar_ts is None:
        ...
        return
    key = f"{event_type}:{bar_ts.isoformat()}"
...
def build_long_rsi35_rebound_prediction(
    ...
    if (not bool(cfg.get("allow_add_position", False))) and rsi_now <= float(cfg.get("add_position_rsi9_max", 19)) and storage:
                ...
                if drained:
                    ...
                    storage.log_structured(
                        ...
                        "WS_QUEUE_DRAINED",
                        {
                            ...
                            "oldest_snapshot_age_ms": max((now_ - drained[0].ts).total_seconds() * 1000.0, 0.0),
                            "newest_snapshot_age_ms": max((now_ - drained[-1].ts).total_seconds() * 1000.0, 0.0),
                        },
                    )
