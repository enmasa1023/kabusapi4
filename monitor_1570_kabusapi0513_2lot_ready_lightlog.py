...
STRATEGY_MODE_LONG_RSI50_AND_RSI35 = "long_rsi50_and_rsi35"
...
LONG_RSI35_ENTRY_RULE = "long_rsi35_rebound"
LONG_RSI35_STRATEGY = "LONG_RSI35_REBOUND"
LONG_RSI35_EXIT_REASON = "LONG_RSI35_REBOUND_RSI_TURN_DOWN_AFTER_50_EXIT"
TAKE_TICKS_1M = 10
TAKE_TICKS_3M = 10
PROB_UPPER_1M = 0.58
PROB_UPPER_3M = 0.54
...
NESTED_CONFIG_DEFAULTS: dict[str, dict[str, Any]] = {
    ...
    "long_rsi50_trend_hold": {
        ...
        "use_rsi50_first_cross_filter": True,
        "allow_morning_initial_rsi50_entry": True,
        "morning_initial_rsi50_entry_cutoff_time": "09:05:00",
    },
    "long_rsi35_rebound": {
        "enabled": True,
        "entry_watch_rsi9_max": 35,
        "entry_trigger": "rsi9_turn_up",
        "exit_watch_rsi9_min": 50,
        "exit_trigger": "rsi9_turn_down_after_50",
        "hard_stop_ticks": 20,
        "allow_add_position": False,
    ...
    "diagnostic_logging": {
        "enabled": True,
        "dedupe_structured_events_per_bar": True,
        "suppress_add_position_disabled_when_disabled": True,
        "ws_queue_drain_summary_enabled": True,
        "ws_queue_drain_summary_interval_sec": 60,
        "ws_queue_drain_log_each_drain": False,
        "gate_decisions_dedupe_per_bar": True,
        "gate_decisions_save_pass_each_loop": False,
        "gate_decisions_save_raw_features_on_pass": False,
        "gate_decisions_save_raw_features_on_block_or_warn": True,
        "gate_decisions_always_save_when_action_not_pass": True,
        "force_close_start_log_once": True,
        "entry_signal_to_order_latency_log_enabled": True,
        "position_reconcile_fast_path_enabled": True,
        "position_reconcile_fast_path_max_age_sec": 10,
    },
...
def startup_config_effective_payload(config: dict[str, Any]) -> dict[str, Any]:
    ...
    rsi35_cfg = long_rsi35_rebound_config(config)
    ...
    diagnostic_cfg = config.get("diagnostic_logging", {}) if isinstance(config.get("diagnostic_logging"), dict) else {}
    return {
        ...
        "morning_trade_start": TRADE_WINDOWS[0][0] if TRADE_WINDOWS else "09:00:00",
        ...
        "long_rsi50_trend_hold_use_rsi50_first_cross_filter": bool(rsi50_cfg.get("use_rsi50_first_cross_filter", True)),
        "long_rsi50_trend_hold_allow_morning_initial_rsi50_entry": bool(rsi50_cfg.get("allow_morning_initial_rsi50_entry", True)),
        "long_rsi50_trend_hold_morning_initial_rsi50_entry_cutoff_time": str(rsi50_cfg.get("morning_initial_rsi50_entry_cutoff_time", "09:05:00")),
        "long_rsi35_rebound_enabled": bool(rsi35_cfg.get("enabled", False)),
        "long_rsi35_rebound_entry_watch_rsi9_max": float(rsi35_cfg.get("entry_watch_rsi9_max", 35)),
        "long_rsi35_rebound_exit_watch_rsi9_min": float(rsi35_cfg.get("exit_watch_rsi9_min", 50)),
        "long_rsi35_rebound_hard_stop_ticks": int(rsi35_cfg.get("hard_stop_ticks", HARD_STOP_TICKS)),
        "long_rsi35_rebound_allow_add_position": bool(rsi35_cfg.get("allow_add_position", False)),
        "long_rsi35_rebound_add_position_rsi9_max": float(rsi35_cfg.get("add_position_rsi9_max", 19)),
        "long_rsi35_rebound_entry_reference_close_guard_enabled": bool(rsi35_cfg.get("entry_reference_close_guard_enabled", True)),
        "long_rsi35_rebound_entry_reference_close_guard_max_abs_deviation_ticks": float(rsi35_cfg.get("entry_reference_close_guard_max_abs_deviation_ticks", 4)),
        ...
        "diagnostic_logging_enabled": bool(diagnostic_cfg.get("enabled", True)),
        "diagnostic_logging_dedupe_structured_events_per_bar": bool(diagnostic_cfg.get("dedupe_structured_events_per_bar", True)),
        "diagnostic_logging_ws_queue_drain_summary_enabled": bool(diagnostic_cfg.get("ws_queue_drain_summary_enabled", True)),
        "diagnostic_logging_ws_queue_drain_summary_interval_sec": int(diagnostic_cfg.get("ws_queue_drain_summary_interval_sec", 60)),
        "diagnostic_logging_gate_decisions_dedupe_per_bar": bool(diagnostic_cfg.get("gate_decisions_dedupe_per_bar", True)),
        "diagnostic_logging_gate_decisions_save_pass_each_loop": bool(diagnostic_cfg.get("gate_decisions_save_pass_each_loop", False)),
        "diagnostic_logging_gate_decisions_save_raw_features_on_pass": bool(diagnostic_cfg.get("gate_decisions_save_raw_features_on_pass", False)),
        "diagnostic_logging_entry_signal_to_order_latency_log_enabled": bool(diagnostic_cfg.get("entry_signal_to_order_latency_log_enabled", True)),
        "diagnostic_logging_position_reconcile_fast_path_enabled": bool(diagnostic_cfg.get("position_reconcile_fast_path_enabled", True)),
...
class MonitorStatus:
    ...
    rsi50_cycle_active: bool = False
    rsi50_cycle_entry_available: bool = False
    rsi50_cycle_consumed: bool = False
    rsi50_cycle_started_bar_ts: Optional[datetime] = None
    rsi50_cycle_started_rsi9: Optional[float] = None
    rsi50_cycle_last_rsi9: Optional[float] = None
    rsi50_morning_initial_entry_used: bool = False
    rsi35_rebound_watch_active: bool = False
    rsi35_rebound_cycle_consumed: bool = False
    rsi35_rebound_started_bar_ts: Optional[datetime] = None
    rsi35_rebound_started_rsi9: Optional[float] = None
    rsi35_rebound_last_rsi9: Optional[float] = None
    rsi35_rebound_exit_watch_active: bool = False
    rsi35_rebound_exit_watch_started_bar_ts: Optional[datetime] = None
    logged_event_bar_keys: set[str] = field(default_factory=set)
    last_gate_decision_saved_bar_ts: Optional[datetime] = None
    last_gate_decision_signature: str = ""
    ws_queue_summary_start_ts: Optional[datetime] = None
    ws_queue_summary_received_start: int = 0
    ws_queue_summary_drained_start: int = 0
    ws_queue_summary_dropped_start: int = 0
    ws_queue_summary_rest_fallback_start: int = 0
    ws_queue_summary_drain_event_count: int = 0
    ws_queue_summary_max_drained_count: int = 0
    ws_queue_summary_max_queue_lag_ms: float = 0.0
    ws_queue_summary_max_oldest_snapshot_age_ms: float = 0.0
    ws_queue_summary_max_newest_snapshot_age_ms: float = 0.0
    ws_queue_summary_last_snapshot_ts: Optional[datetime] = None
    ws_queue_summary_last_ws_seq: Optional[int] = None
    force_market_close_start_logged: bool = False
    last_reconcile_ok_ts: Optional[datetime] = None
    last_reconcile_live_state: str = ""
    last_reconcile_open_orders_count: int = 0
    last_reconcile_positions_count: int = 0
    last_reconcile_verified_flat: bool = False
...
class Storage:
    ...
    def insert_gate_decision(self, symbol: str, pred: PredictionSnapshot, decision: GateDecision) -> None:
        ...
        raw_features_json = None if action in {"", "PASS", "ALLOW", "OK"} else decision_features_json(gf)
        with self._connect() as con:
            con.execute(
...
class WebSocketMarketDataFeed:
    ...
    def __init__(self, base_url: str, storage: Optional[Storage] = None, queue_maxlen: int = 5000) -> None:
        self.base_url = base_url.rstrip("/")
        self.storage = storage
        self._latest: Optional[TickSnapshot] = None
        self._queue: deque[TickSnapshot] = deque(maxlen=max(int(queue_maxlen), 1))
        self._queue_maxlen = max(int(queue_maxlen), 1)
    ...
    def wait_for_data(self, timeout_sec: float) -> bool:
        got_data = self._data_event.wait(max(float(timeout_sec), 0.0))
        if got_data:
    ...
    def metrics(self) -> dict[str, Any]:
        with self._lock:
            return {
                ...
                "queue_maxlen": self._queue_maxlen,
        ...
        def on_message(_ws: Any, message: str) -> None:
            try:
                ...
                with self._lock:
                    ...
                    self._last_received_ts = received_ts
                    self._last_enqueued_snapshot_ts = snap.ts
                self.available = True
...
def long_rsi35_rebound_config(config: dict[str, Any]) -> dict[str, Any]:
    raw = config.get("long_rsi35_rebound", {}) if isinstance(config.get("long_rsi35_rebound"), dict) else {}
    return deep_merge_dict(NESTED_CONFIG_DEFAULTS["long_rsi35_rebound"], raw)
...
def is_long_rsi50_and_rsi35_mode(config: dict[str, Any]) -> bool:
    return str(config.get("strategy_mode", "legacy")) == STRATEGY_MODE_LONG_RSI50_AND_RSI35


def is_long_rsi_managed_mode(config: dict[str, Any]) -> bool:
    return is_long_rsi50_trend_hold_only(config) or is_long_rsi50_and_rsi35_mode(config)


def log_structured_once_per_bar(
    storage: Optional[Storage],
    ...
    if storage is None:
        return False
    if status is None or bar_ts is None:
        storage.log_structured(level, event_type, payload)
        return True
    key = f"{event_type}::{bar_ts.isoformat()}::{dedupe_extra}"
    if key in status.logged_event_bar_keys:
        return False
    status.logged_event_bar_keys.add(key)
    if correlation_id:
...
def reset_rsi35_rebound_cycle(
    ...
    if not (
        status.rsi35_rebound_watch_active
        or status.rsi35_rebound_cycle_consumed
        or status.rsi35_rebound_exit_watch_active
    ):
        status.rsi35_rebound_last_rsi9 = rsi9
        return
    status.rsi35_rebound_watch_active = False
    status.rsi35_rebound_cycle_consumed = False
    status.rsi35_rebound_started_bar_ts = None
    status.rsi35_rebound_started_rsi9 = None
    status.rsi35_rebound_exit_watch_active = False
    status.rsi35_rebound_exit_watch_started_bar_ts = None
    status.rsi35_rebound_last_rsi9 = rsi9
    if storage is not None:
        storage.log_structured(
            "INFO",
            "LONG_RSI35_REBOUND_CYCLE_RESET",
            {
                "ts": ts.isoformat(),
                ...
                "exit_deviation_ticks": None,
                "entry_watch_rsi9_max": None,
                "exit_watch_rsi9_min": None,
...
def should_save_gate_decision(
    status: MonitorStatus,
    config: dict[str, Any],
    pred: PredictionSnapshot,
    decision: GateDecision,
) -> bool:
    diag = config.get("diagnostic_logging", {}) if isinstance(config.get("diagnostic_logging"), dict) else {}
    if not bool(diag.get("gate_decisions_dedupe_per_bar", True)):
        return True
    action = str(decision.action or "").upper()
    reason = str(decision.reason or "")
    signature = f"GATE_DECISION::{pred.ts.isoformat()}::{decision.final_signal}::{action}::{reason}"
    non_pass = action not in {"", "PASS", "ALLOW", "OK"}
    if non_pass and bool(diag.get("gate_decisions_always_save_when_action_not_pass", True)):
        if signature != status.last_gate_decision_signature:
            status.last_gate_decision_signature = signature
            status.last_gate_decision_saved_bar_ts = pred.ts
            return True
        return False
    if bool(diag.get("gate_decisions_save_pass_each_loop", False)):
        return True
    if pred.ts != status.last_gate_decision_saved_bar_ts or signature != status.last_gate_decision_signature:
        status.last_gate_decision_signature = signature
        status.last_gate_decision_saved_bar_ts = pred.ts
        return True
    return False
...
def can_use_position_reconcile_fast_path(status: MonitorStatus, config: dict[str, Any], now_: datetime) -> tuple[bool, str, Optional[float]]:
    diag = config.get("diagnostic_logging", {}) if isinstance(config.get("diagnostic_logging"), dict) else {}
    if not bool(diag.get("position_reconcile_fast_path_enabled", True)):
        return False, "DISABLED", None
    if status.open_position is not None or status.live_state != "FLAT" or status.pending_entry_side is not None or status.pending_exit or status.pending_add:
        return False, "UNSAFE_STATE", None
    if status.recovery_until and now_ < status.recovery_until:
        return False, "RECOVERY_COOLDOWN", None
    if status.last_reconcile_ok_ts is None:
        return False, "NO_RECENT_RECONCILE", None
    age_sec = (now_ - status.last_reconcile_ok_ts).total_seconds()
    if age_sec > float(diag.get("position_reconcile_fast_path_max_age_sec", 10)):
        return False, "RECENT_RECONCILE_TOO_OLD", age_sec
    if not status.last_reconcile_verified_flat or status.last_reconcile_open_orders_count != 0 or status.last_reconcile_positions_count != 0:
        return False, "LAST_RECONCILE_NOT_VERIFIED_FLAT", age_sec
    return True, "OK", age_sec


def log_entry_signal_to_order_latency(
    storage: Storage,
    config: dict[str, Any],
    candidate_pos: PositionState,
    side: str,
    pred: PredictionSnapshot,
    trigger_ts: datetime,
    order_request_ts: Optional[datetime],
    position_reconcile_started_ts: Optional[datetime],
    position_reconcile_finished_ts: Optional[datetime],
    pre_send_snapshot_started_ts: Optional[datetime],
    pre_send_snapshot_finished_ts: Optional[datetime],
    reference_close: Optional[float],
    best_ask: Optional[float],
    best_bid: Optional[float],
    entry_deviation_ticks: Optional[float],
    block_reason: str,
    used_position_reconcile_fast_path: bool,
    status: MonitorStatus,
) -> None:
    diag = config.get("diagnostic_logging", {}) if isinstance(config.get("diagnostic_logging"), dict) else {}
    if not bool(diag.get("entry_signal_to_order_latency_log_enabled", True)):
        return
    end_ts = order_request_ts or now_jst()
    storage.log_structured("INFO", "ENTRY_SIGNAL_TO_ORDER_LATENCY", {
        "strategy": candidate_pos.strategy,
        "entry_rule": candidate_pos.entry_rule,
        "side": side,
        "bar_ts": pred.ts.isoformat(),
        "trigger_ts": trigger_ts.isoformat(),
        "order_request_ts": order_request_ts.isoformat() if order_request_ts else None,
        "elapsed_ms": (end_ts - trigger_ts).total_seconds() * 1000.0,
        "position_reconcile_started_ts": position_reconcile_started_ts.isoformat() if position_reconcile_started_ts else None,
        "position_reconcile_finished_ts": position_reconcile_finished_ts.isoformat() if position_reconcile_finished_ts else None,
        "position_reconcile_elapsed_ms": ((position_reconcile_finished_ts - position_reconcile_started_ts).total_seconds() * 1000.0 if position_reconcile_started_ts and position_reconcile_finished_ts else None),
        "pre_send_snapshot_started_ts": pre_send_snapshot_started_ts.isoformat() if pre_send_snapshot_started_ts else None,
        "pre_send_snapshot_finished_ts": pre_send_snapshot_finished_ts.isoformat() if pre_send_snapshot_finished_ts else None,
        "pre_send_snapshot_elapsed_ms": ((pre_send_snapshot_finished_ts - pre_send_snapshot_started_ts).total_seconds() * 1000.0 if pre_send_snapshot_started_ts and pre_send_snapshot_finished_ts else None),
        "reference_close": reference_close,
        "best_ask": best_ask,
        "best_bid": best_bid,
        "entry_deviation_ticks": entry_deviation_ticks,
        "block_reason": block_reason,
        "used_position_reconcile_fast_path": used_position_reconcile_fast_path,
        "live_state": status.live_state,
        "pending_entry": status.pending_entry_side,
        "pending_exit": status.pending_exit,
    })


def record_ws_queue_drain_summary(
    status: MonitorStatus,
    storage: Storage,
    config: dict[str, Any],
    now_: datetime,
    ...
    diag = config.get("diagnostic_logging", {}) if isinstance(config.get("diagnostic_logging"), dict) else {}
    if not bool(diag.get("ws_queue_drain_summary_enabled", True)):
        return
    interval = float(diag.get("ws_queue_drain_summary_interval_sec", 60))
    received_total = int(ws_metrics.get("received_count_total") or 0)
    drained_total = int(ws_metrics.get("drained_count_total") or 0)
    dropped_total = int(ws_metrics.get("dropped_count_total") or 0)
    if status.ws_queue_summary_start_ts is None:
        status.ws_queue_summary_start_ts = now_
        status.ws_queue_summary_received_start = received_total
        status.ws_queue_summary_drained_start = drained_total
        status.ws_queue_summary_dropped_start = dropped_total
    status.ws_queue_summary_drain_event_count += 1
    status.ws_queue_summary_max_drained_count = max(status.ws_queue_summary_max_drained_count, len(drained))
    status.ws_queue_summary_max_queue_lag_ms = max(status.ws_queue_summary_max_queue_lag_ms, oldest_age_ms)
    status.ws_queue_summary_max_oldest_snapshot_age_ms = max(status.ws_queue_summary_max_oldest_snapshot_age_ms, oldest_age_ms)
    status.ws_queue_summary_max_newest_snapshot_age_ms = max(status.ws_queue_summary_max_newest_snapshot_age_ms, newest_age_ms)
    status.ws_queue_summary_last_snapshot_ts = drained[-1].ts if drained else status.ws_queue_summary_last_snapshot_ts
    status.ws_queue_summary_last_ws_seq = drained[-1].ws_seq if drained and drained[-1].ws_seq is not None else status.ws_queue_summary_last_ws_seq
    if (now_ - status.ws_queue_summary_start_ts).total_seconds() >= interval:
        storage.log_structured("INFO", "WS_QUEUE_DRAINED_SUMMARY", {
            "summary_start_ts": status.ws_queue_summary_start_ts.isoformat(),
            "summary_end_ts": now_.isoformat(),
            "received_count_delta": received_total - status.ws_queue_summary_received_start,
            "drained_count_delta": drained_total - status.ws_queue_summary_drained_start,
            "dropped_count_delta": dropped_total - status.ws_queue_summary_dropped_start,
            "dropped_count_total": dropped_total,
            "drain_event_count": status.ws_queue_summary_drain_event_count,
            "max_drained_count": status.ws_queue_summary_max_drained_count,
            "max_queue_lag_ms": status.ws_queue_summary_max_queue_lag_ms,
            "max_oldest_snapshot_age_ms": status.ws_queue_summary_max_oldest_snapshot_age_ms,
            "max_newest_snapshot_age_ms": status.ws_queue_summary_max_newest_snapshot_age_ms,
            "last_snapshot_ts": status.ws_queue_summary_last_snapshot_ts.isoformat() if status.ws_queue_summary_last_snapshot_ts else None,
            "last_ws_seq": status.ws_queue_summary_last_ws_seq,
            "rest_fallback_count_delta": 0,
        })
        status.ws_queue_summary_start_ts = now_
        status.ws_queue_summary_received_start = received_total
        status.ws_queue_summary_drained_start = drained_total
        status.ws_queue_summary_dropped_start = dropped_total
        status.ws_queue_summary_drain_event_count = 0
        status.ws_queue_summary_max_drained_count = 0
        status.ws_queue_summary_max_queue_lag_ms = 0.0
        status.ws_queue_summary_max_oldest_snapshot_age_ms = 0.0
        status.ws_queue_summary_max_newest_snapshot_age_ms = 0.0
...
def build_long_rsi50_trend_hold_prediction(
    ...
    allow_new_entry: bool = True,
    new_entry_cutoff_reached: bool = False,
    prev_rsi9_for_decision: Optional[float] = None,
) -> Optional[PredictionSnapshot]:
    if bar1 is None:
        return None
    closes = [b.close for b in history]
    ...
    use_first_cross_filter = bool(cfg.get("use_rsi50_first_cross_filter", True))
    prev_rsi9_for_cycle = prev_rsi9_for_decision if prev_rsi9_for_decision is not None else (status.rsi50_cycle_last_rsi9 if status is not None else None)
    cycle_allows_entry = True
    cycle_block_reason = ""
    morning_initial_eligible = False
    if status is not None and use_first_cross_filter:
        if rsi_now <= exit_rsi9_max:
            if status.rsi50_cycle_active or status.rsi50_cycle_consumed:
                storage and storage.log_structured("INFO", "LONG_RSI50_CYCLE_RESET_BY_RSI49", {
                    "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now,
                    "prev_rsi9": prev_rsi9_for_cycle, "entry_rsi9_min": entry_rsi9_min, "exit_rsi9_max": exit_rsi9_max,
                })
            status.rsi50_cycle_active = False
            status.rsi50_cycle_entry_available = False
            status.rsi50_cycle_consumed = False
            status.rsi50_cycle_started_bar_ts = None
            status.rsi50_cycle_started_rsi9 = None
        elif rsi_now >= entry_rsi9_min and not status.rsi50_cycle_active:
            crossed_from_below = prev_rsi9_for_cycle is not None and prev_rsi9_for_cycle <= exit_rsi9_max
            cutoff = str(cfg.get("morning_initial_rsi50_entry_cutoff_time", "09:05:00"))
            morning_initial_eligible = (
                prev_rsi9_for_cycle is None
                and bool(cfg.get("allow_morning_initial_rsi50_entry", True))
                and not status.rsi50_morning_initial_entry_used
                and bar1.ts.strftime("%H:%M:%S") <= cutoff
                and open_pos is None
            )
            if crossed_from_below or morning_initial_eligible:
                status.rsi50_cycle_active = True
                status.rsi50_cycle_entry_available = True
                status.rsi50_cycle_consumed = False
                status.rsi50_cycle_started_bar_ts = bar1.ts
                status.rsi50_cycle_started_rsi9 = rsi_now
                event_type = "LONG_RSI50_MORNING_INITIAL_ENTRY_ELIGIBLE" if morning_initial_eligible else "LONG_RSI50_CYCLE_STARTED"
                log_structured_once_per_bar(storage, status, "INFO", event_type, {
                    "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now,
                    "prev_rsi9_for_decision": prev_rsi9_for_cycle, "current_rsi9": rsi_now,
                    "prev_rsi9": prev_rsi9_for_cycle, "entry_rsi9_min": entry_rsi9_min, "exit_rsi9_max": exit_rsi9_max,
                    "ema13": ema13_now, "ema13_lookback_value": ema13_lookback_value, "ema13_delta_ticks": ema13_delta_ticks,
                    "reference_close": bar1.close, "best_ask": best_ask, "entry_deviation_ticks": entry_deviation_ticks,
                    "cycle_active": status.rsi50_cycle_active, "cycle_entry_available": status.rsi50_cycle_entry_available,
                    "cycle_consumed": status.rsi50_cycle_consumed, "block_reason": "", "signal": "CYCLE_ENTRY_ELIGIBLE",
                    "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason},
                }, bar1.ts)
                log_structured_once_per_bar(storage, status, "INFO", "LONG_RSI50_CYCLE_ENTRY_ELIGIBLE", {
                    "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now,
                    "prev_rsi9_for_decision": prev_rsi9_for_cycle, "current_rsi9": rsi_now,
                    "prev_rsi9": prev_rsi9_for_cycle, "entry_rsi9_min": entry_rsi9_min, "exit_rsi9_max": exit_rsi9_max,
                    "ema13": ema13_now, "ema13_lookback_value": ema13_lookback_value, "ema13_delta_ticks": ema13_delta_ticks,
        ...
        cycle_allows_entry = bool(status.rsi50_cycle_entry_available and not status.rsi50_cycle_consumed)
        if rsi_now >= entry_rsi9_min and not cycle_allows_entry:
            cycle_block_reason = "RSI50_CYCLE_ALREADY_USED"
            log_structured_once_per_bar(storage, status, "INFO", "LONG_RSI50_ENTRY_BLOCKED_BY_RSI50_CYCLE_ALREADY_USED", {
                "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now,
                "prev_rsi9_for_decision": prev_rsi9_for_cycle, "current_rsi9": rsi_now,
                "prev_rsi9": prev_rsi9_for_cycle, "entry_rsi9_min": entry_rsi9_min, "exit_rsi9_max": exit_rsi9_max,
                "ema13": ema13_now, "ema13_lookback_value": ema13_lookback_value, "ema13_delta_ticks": ema13_delta_ticks,
                "reference_close": bar1.close, "best_ask": best_ask, "entry_deviation_ticks": entry_deviation_ticks,
                "cycle_active": status.rsi50_cycle_active, "cycle_entry_available": status.rsi50_cycle_entry_available,
                "cycle_consumed": status.rsi50_cycle_consumed, "block_reason": cycle_block_reason, "signal": "NO_ACTION",
                "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason},
            }, bar1.ts)
    entry_ok = (
        ...
        and cycle_allows_entry
        and ema13_ok
        and reference_close_ok
    )
    if status is not None and use_first_cross_filter and rsi_now >= entry_rsi9_min and cycle_allows_entry:
        if entry_ok:
            status.rsi50_cycle_consumed = True
            status.rsi50_cycle_entry_available = False
            if morning_initial_eligible:
                status.rsi50_morning_initial_entry_used = True
                storage and storage.log_structured("INFO", "LONG_RSI50_MORNING_INITIAL_ENTRY_USED", {"ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi9_for_cycle, "block_reason": "", "signal": "LONG_CANDIDATE", "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason}})
            storage and storage.log_structured("INFO", "LONG_RSI50_CYCLE_ENTRY_CONSUMED_BY_TRIGGER", {"ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi9_for_cycle, "entry_rsi9_min": entry_rsi9_min, "exit_rsi9_max": exit_rsi9_max, "ema13": ema13_now, "ema13_lookback_value": ema13_lookback_value, "ema13_delta_ticks": ema13_delta_ticks, "reference_close": bar1.close, "best_ask": best_ask, "entry_deviation_ticks": entry_deviation_ticks, "cycle_active": status.rsi50_cycle_active, "cycle_entry_available": status.rsi50_cycle_entry_available, "cycle_consumed": status.rsi50_cycle_consumed, "block_reason": "", "signal": "LONG_CANDIDATE", "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason}})
        elif status.rsi50_cycle_entry_available:
            status.rsi50_cycle_consumed = True
            status.rsi50_cycle_entry_available = False
            if morning_initial_eligible:
                status.rsi50_morning_initial_entry_used = True
                storage and storage.log_structured("INFO", "LONG_RSI50_MORNING_INITIAL_ENTRY_USED", {"ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi9_for_cycle, "block_reason": "ENTRY_BLOCKED", "signal": "NO_ACTION", "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason}})
            storage and storage.log_structured("INFO", "LONG_RSI50_CYCLE_ENTRY_CONSUMED_BY_BLOCK", {"ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi9_for_cycle, "entry_rsi9_min": entry_rsi9_min, "exit_rsi9_max": exit_rsi9_max, "ema13": ema13_now, "ema13_lookback_value": ema13_lookback_value, "ema13_delta_ticks": ema13_delta_ticks, "reference_close": bar1.close, "best_ask": best_ask, "entry_deviation_ticks": entry_deviation_ticks, "cycle_active": status.rsi50_cycle_active, "cycle_entry_available": status.rsi50_cycle_entry_available, "cycle_consumed": status.rsi50_cycle_consumed, "block_reason": ema13_block_reason or ("REFERENCE_CLOSE_DEVIATION_TOO_LARGE" if not reference_close_ok else "ENTRY_BLOCKED"), "signal": "NO_ACTION", "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason}})
    # status.rsi50_cycle_last_rsi9 is updated by the main loop after all
    # entry/exit decisions for this finalized bar have used the previous value.
...
def build_long_rsi35_rebound_prediction(
    bar1: Optional[Bar],
    history: list[Bar],
    open_pos: Optional[PositionState],
    status: Optional[MonitorStatus],
    ...
    new_entry_cutoff_reached: bool = False,
    prev_rsi9_for_decision: Optional[float] = None,
    current_rsi9: Optional[float] = None,
) -> Optional[PredictionSnapshot]:
    if bar1 is None or status is None:
        return None
    closes = [b.close for b in history]
    if len(closes) <= RSI9_PERIOD + 1:
        return None
    rsi_now = current_rsi9 if current_rsi9 is not None else rsi9_wilder(closes, RSI9_PERIOD)
    rsi_prev = prev_rsi9_for_decision
    if rsi_now is None:
        return None
    cfg = long_rsi35_rebound_config(config)
    entry_watch_max = float(cfg.get("entry_watch_rsi9_max", 35))
    exit_watch_min = float(cfg.get("exit_watch_rsi9_min", 50))
    best_ask = latest_snapshot.sell1_price if latest_snapshot is not None else None
    guard_enabled = bool(cfg.get("entry_reference_close_guard_enabled", True))
    max_deviation_ticks = float(cfg.get("entry_reference_close_guard_max_abs_deviation_ticks", 4))
    entry_deviation_ticks = reference_close_deviation_ticks(best_ask, bar1.close)
    reference_ok = (not guard_enabled) or (entry_deviation_ticks is not None and entry_deviation_ticks <= max_deviation_ticks)

    if rsi_now <= entry_watch_max and not status.rsi35_rebound_cycle_consumed:
        if not status.rsi35_rebound_watch_active:
            status.rsi35_rebound_watch_active = True
            status.rsi35_rebound_started_bar_ts = bar1.ts
            status.rsi35_rebound_started_rsi9 = rsi_now
            if storage:
                log_structured_once_per_bar(storage, status, "INFO", "LONG_RSI35_REBOUND_WATCH_STARTED", {
                    "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": rsi_prev,
                    "prev_rsi9_for_decision": rsi_prev, "current_rsi9": rsi_now,
                    "close": bar1.close, "best_ask": best_ask, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None,
                    "reference_close": bar1.close, "entry_deviation_ticks": entry_deviation_ticks, "exit_deviation_ticks": None,
                    "entry_watch_rsi9_max": entry_watch_max, "exit_watch_rsi9_min": exit_watch_min,
                    "position_entry_price": None, "unrealized_ticks": None,
                    "cycle_state": {"watch_active": True, "consumed": status.rsi35_rebound_cycle_consumed},
                    "block_reason": "", "signal": "WATCH_STARTED", "can_enter": None,
                }, bar1.ts)
    turn_up = bool(status.rsi35_rebound_watch_active and rsi_prev is not None and rsi_now > rsi_prev)
    can_enter_ok, can_enter_reason = can_enter("LONG", bar1.ts, status)
    pending_entry = status.pending_entry_side is not None
    pending_exit = status.pending_exit
    entry_ok = (
        bool(cfg.get("enabled", True))
        and open_pos is None
        ...
        and allow_new_entry
        and not new_entry_cutoff_reached
        and not status.rsi35_rebound_cycle_consumed
        and turn_up
        and reference_ok
        and can_enter_ok
        and status.live_state not in {"RECOVERING", "MANUAL_POSITION_CHECK_REQUIRED", "ENTRY_SENT", "EXIT_SENT", "EXIT_VERIFYING"}
    )
    if storage and status.rsi35_rebound_watch_active:
        log_structured_once_per_bar(storage, status, "INFO", "LONG_RSI35_REBOUND_ENTRY_EVALUATED", {
            "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": rsi_prev,
            "prev_rsi9_for_decision": rsi_prev, "current_rsi9": rsi_now,
            "close": bar1.close, "best_ask": best_ask, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None,
            "reference_close": bar1.close, "entry_deviation_ticks": entry_deviation_ticks, "exit_deviation_ticks": None,
            "entry_watch_rsi9_max": entry_watch_max, "exit_watch_rsi9_min": exit_watch_min,
            "position_entry_price": None, "unrealized_ticks": None,
            "cycle_state": {"watch_active": status.rsi35_rebound_watch_active, "consumed": status.rsi35_rebound_cycle_consumed},
            "block_reason": "" if entry_ok else ("REFERENCE_CLOSE_DEVIATION_TOO_LARGE" if turn_up and not reference_ok else "NOT_TRIGGERED"),
            "signal": "LONG_CANDIDATE" if entry_ok else "NO_ACTION", "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason},
        }, bar1.ts)
    if entry_ok:
        status.rsi35_rebound_cycle_consumed = True
        if storage:
            storage.log_structured("INFO", "LONG_RSI35_REBOUND_ENTRY_TRIGGERED", {
                "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": rsi_prev,
                "prev_rsi9_for_decision": rsi_prev, "current_rsi9": rsi_now,
                "close": bar1.close, "best_ask": best_ask, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None,
                "reference_close": bar1.close, "entry_deviation_ticks": entry_deviation_ticks, "exit_deviation_ticks": None,
                ...
                "position_entry_price": None, "unrealized_ticks": None,
                "cycle_state": {"watch_active": status.rsi35_rebound_watch_active, "consumed": status.rsi35_rebound_cycle_consumed},
                "block_reason": "", "signal": "LONG_CANDIDATE", "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason},
            })
    elif turn_up and not status.rsi35_rebound_cycle_consumed:
        status.rsi35_rebound_cycle_consumed = True
        event = "LONG_RSI35_REBOUND_ENTRY_BLOCKED_BY_REFERENCE_CLOSE_DEVIATION" if not reference_ok else "LONG_RSI35_REBOUND_ENTRY_CONSUMED_BY_BLOCK"
        if storage:
            storage.log_structured("INFO", event, {
                "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": rsi_prev,
                "close": bar1.close, "best_ask": best_ask, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None,
                "reference_close": bar1.close, "entry_deviation_ticks": entry_deviation_ticks, "exit_deviation_ticks": None,
                "entry_watch_rsi9_max": entry_watch_max, "exit_watch_rsi9_min": exit_watch_min,
                "position_entry_price": None, "unrealized_ticks": None,
                "cycle_state": {"watch_active": status.rsi35_rebound_watch_active, "consumed": status.rsi35_rebound_cycle_consumed},
                "block_reason": "REFERENCE_CLOSE_DEVIATION_TOO_LARGE" if not reference_ok else "ENTRY_BLOCKED",
                "signal": "NO_ACTION", "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason},
            })
    elif turn_up and status.rsi35_rebound_cycle_consumed and storage:
        storage.log_structured("INFO", "LONG_RSI35_REBOUND_ENTRY_BLOCKED_BY_ALREADY_USED", {
            "ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": rsi_prev,
            "close": bar1.close, "best_ask": best_ask, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None,
            "reference_close": bar1.close, "entry_deviation_ticks": entry_deviation_ticks, "exit_deviation_ticks": None,
            "entry_watch_rsi9_max": entry_watch_max, "exit_watch_rsi9_min": exit_watch_min,
            "position_entry_price": None, "unrealized_ticks": None,
            "cycle_state": {"watch_active": status.rsi35_rebound_watch_active, "consumed": status.rsi35_rebound_cycle_consumed},
            "block_reason": "RSI35_REBOUND_CYCLE_ALREADY_USED",
            "signal": "NO_ACTION", "can_enter": {"ok": can_enter_ok, "reason": can_enter_reason},
        })
    diag_cfg = config.get("diagnostic_logging", {}) if isinstance(config.get("diagnostic_logging"), dict) else {}
    if (
        (not bool(diag_cfg.get("suppress_add_position_disabled_when_disabled", True)))
        and (not bool(cfg.get("allow_add_position", False)))
        and rsi_now <= float(cfg.get("add_position_rsi9_max", 19))
        and storage
    ):
        storage.log_structured("INFO", "LONG_RSI35_REBOUND_ADD_POSITION_DISABLED", {"ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": rsi_prev, "block_reason": "allow_add_position_false", "signal": "NO_ACTION", "can_enter": None})
    if rsi_now >= exit_watch_min and open_pos is None and status.rsi35_rebound_cycle_consumed:
        status.rsi35_rebound_watch_active = False
        status.rsi35_rebound_cycle_consumed = False
        status.rsi35_rebound_started_bar_ts = None
        status.rsi35_rebound_started_rsi9 = None
        storage and storage.log_structured("INFO", "LONG_RSI35_REBOUND_CYCLE_RESET", {"ts": feature.ts.isoformat(), "bar_ts": bar1.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": rsi_prev, "block_reason": "RSI_REACHED_EXIT_WATCH_WITHOUT_POSITION", "signal": "NO_ACTION", "can_enter": None})
    # status.rsi35_rebound_last_rsi9 is updated by the main loop after both
    # RSI35 entry and exit decisions have used prev_rsi9_for_decision.
    return PredictionSnapshot(
        ts=bar1.ts,
        regime=LONG_RSI35_STRATEGY,
        p_up_1m=0.5,
        p_down_1m=0.5,
        p_up_3m=0.5,
        p_down_3m=0.5,
        signal="LONG_CANDIDATE" if entry_ok else "NO_ACTION",
        rsi9_value=rsi_now,
        reason_1=LONG_RSI35_STRATEGY,
        reason_2=f"rsi9={rsi_now:.2f}",
        reason_3=LONG_RSI35_ENTRY_RULE if entry_ok else "none",
    )
...
def create_position(
    ...
    elif pred.reason_1 == LONG_RSI35_STRATEGY or pred.reason_3 == LONG_RSI35_ENTRY_RULE:
        strategy = LONG_RSI35_STRATEGY
        hard_stop = int(long_rsi35_rebound_config(config).get("hard_stop_ticks", config.get("hard_stop_ticks", HARD_STOP_TICKS)))
        stop_ticks, take_ticks, min_hold, max_hold = hard_stop, 0, 0, 3600
    ...
    return PositionState(
        ...
        filled_qty=0,
        remaining_qty=0,
        hard_stop_ticks=stop_ticks if strategy in SCALP_FEATURE_STRATEGIES or strategy in {LONG_RSI50_STRATEGY, LONG_RSI35_STRATEGY} else int(config.get("hard_stop_ticks", HARD_STOP_TICKS)),
    )
...
def should_exit(
    ...
    if pos.strategy == LONG_RSI35_STRATEGY:
        pnl_ticks = current_pnl_ticks(pos, f.price)
        if pnl_ticks <= -pos.hard_stop_ticks:
            if storage is not None:
                storage.log_structured(
                    "WARN",
                    "LONG_RSI35_REBOUND_HARD_STOP_TRIGGERED",
                    {
                        "ts": f.ts.isoformat(),
                        "bar_ts": bar1_new.ts.isoformat() if bar1_new is not None else None,
                        "rsi9": pred.rsi9_value,
                        "prev_rsi9": prev_rsi9_for_decision,
                        "prev_rsi9_for_decision": prev_rsi9_for_decision,
                        "current_rsi9": current_rsi9 if current_rsi9 is not None else pred.rsi9_value,
                        "close": bar1_new.close if bar1_new is not None else None,
                        "best_ask": latest_snapshot.sell1_price if latest_snapshot is not None else None,
                        "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None,
                        "reference_close": bar1_new.close if bar1_new is not None else None,
                        "entry_deviation_ticks": None,
                        "exit_deviation_ticks": None,
                        "entry_watch_rsi9_max": float(long_rsi35_rebound_config(config or {}).get("entry_watch_rsi9_max", 35)),
                        "exit_watch_rsi9_min": float(long_rsi35_rebound_config(config or {}).get("exit_watch_rsi9_min", 50)),
                        "position_entry_price": pos.entry_price,
                        "unrealized_ticks": pnl_ticks,
                        "cycle_state": {
                            "watch_active": status.rsi35_rebound_watch_active if status is not None else None,
                            "cycle_consumed": status.rsi35_rebound_cycle_consumed if status is not None else None,
                            "exit_watch_active": status.rsi35_rebound_exit_watch_active if status is not None else None,
                        },
                        "block_reason": "",
                        "signal": "EXIT",
                        "can_enter": None,
            ...
            return True, "HARD_STOP_LOSS", pnl_ticks
        cfg35 = long_rsi35_rebound_config(config or {})
        exit_watch_rsi9_min = float(cfg35.get("exit_watch_rsi9_min", 50))
        rsi_now = current_rsi9 if current_rsi9 is not None else pred.rsi9_value
        prev_rsi = prev_rsi9_for_decision
        if bar1_new is not None and rsi_now is not None:
            if status is not None and rsi_now >= exit_watch_rsi9_min and not status.rsi35_rebound_exit_watch_active:
                status.rsi35_rebound_exit_watch_active = True
                status.rsi35_rebound_exit_watch_started_bar_ts = bar1_new.ts
                if storage is not None:
                    log_structured_once_per_bar(storage, status, "INFO", "LONG_RSI35_REBOUND_EXIT_WATCH_STARTED", {"ts": f.ts.isoformat(), "bar_ts": bar1_new.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi, "prev_rsi9_for_decision": prev_rsi, "current_rsi9": rsi_now, "close": bar1_new.close, "best_ask": latest_snapshot.sell1_price if latest_snapshot is not None else None, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None, "reference_close": bar1_new.close, "entry_deviation_ticks": None, "exit_deviation_ticks": None, "entry_watch_rsi9_max": float(cfg35.get("entry_watch_rsi9_max", 35)), "exit_watch_rsi9_min": exit_watch_rsi9_min, "position_entry_price": pos.entry_price, "unrealized_ticks": pnl_ticks, "cycle_state": {"exit_watch_active": True}, "exit_watch_active": True, "triggered": False, "reason": "EXIT_WATCH_STARTED", "block_reason": "", "signal": "EXIT_WATCH", "can_enter": None}, bar1_new.ts)
            exit_triggered = bool(status is not None and status.rsi35_rebound_exit_watch_active and prev_rsi is not None and rsi_now < prev_rsi)
            if storage is not None and status is not None and status.rsi35_rebound_exit_watch_active:
                log_structured_once_per_bar(storage, status, "INFO", "LONG_RSI35_REBOUND_EXIT_EVALUATED", {"ts": f.ts.isoformat(), "bar_ts": bar1_new.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi, "prev_rsi9_for_decision": prev_rsi, "current_rsi9": rsi_now, "close": bar1_new.close, "best_ask": latest_snapshot.sell1_price if latest_snapshot is not None else None, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None, "reference_close": bar1_new.close, "entry_deviation_ticks": None, "exit_deviation_ticks": reference_close_deviation_ticks(latest_snapshot.buy1_price if latest_snapshot is not None else None, bar1_new.close), "entry_watch_rsi9_max": float(cfg35.get("entry_watch_rsi9_max", 35)), "exit_watch_rsi9_min": exit_watch_rsi9_min, "position_entry_price": pos.entry_price, "unrealized_ticks": pnl_ticks, "cycle_state": {"exit_watch_active": status.rsi35_rebound_exit_watch_active}, "exit_watch_active": status.rsi35_rebound_exit_watch_active, "triggered": exit_triggered, "reason": "RSI_TURN_DOWN_AFTER_50" if exit_triggered else "RSI_NOT_TURNED_DOWN_AFTER_50", "block_reason": "" if exit_triggered else "RSI_NOT_TURNED_DOWN_AFTER_50", "signal": "EXIT" if exit_triggered else "HOLD", "can_enter": None}, bar1_new.ts)
            if exit_triggered:
                if storage is not None:
                    storage.log_structured("INFO", "LONG_RSI35_REBOUND_EXIT_TRIGGERED", {"ts": f.ts.isoformat(), "bar_ts": bar1_new.ts.isoformat(), "rsi9": rsi_now, "prev_rsi9": prev_rsi, "prev_rsi9_for_decision": prev_rsi, "current_rsi9": rsi_now, "close": bar1_new.close, "best_ask": latest_snapshot.sell1_price if latest_snapshot is not None else None, "best_bid": latest_snapshot.buy1_price if latest_snapshot is not None else None, "reference_close": bar1_new.close, "entry_deviation_ticks": None, "exit_deviation_ticks": reference_close_deviation_ticks(latest_snapshot.buy1_price if latest_snapshot is not None else None, bar1_new.close), "entry_watch_rsi9_max": float(cfg35.get("entry_watch_rsi9_max", 35)), "exit_watch_rsi9_min": exit_watch_rsi9_min, "position_entry_price": pos.entry_price, "unrealized_ticks": pnl_ticks, "cycle_state": {"exit_watch_active": True}, "exit_watch_active": True, "triggered": True, "reason": "RSI_TURN_DOWN_AFTER_50", "block_reason": "", "signal": "EXIT", "can_enter": None, "exit_reason": LONG_RSI35_EXIT_REASON})
                return True, LONG_RSI35_EXIT_REASON, pnl_ticks
        return False, "HOLD", pnl_ticks
...
def positions_summary_for_log(positions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for pos in positions:
        execution_id = pos.get("ExecutionID") or pos.get("HoldID")
        leaves_qty = pos.get("LeavesQty")
        hold_qty = pos.get("HoldQty")
        available_qty = pos.get("AvailableQty")
        if available_qty is None and leaves_qty is not None:
            available_qty = max(_to_int(leaves_qty, 0) - _to_int(hold_qty, 0), 0)
        item = {
            "ExecutionID": execution_id,
            "HoldID": pos.get("HoldID") or execution_id,
            "Qty": pos.get("Qty") if pos.get("Qty") is not None else leaves_qty,
...
def log_managed_close_positions_built(
    storage: Storage,
    source: str,
    managed_execution_ids: list[str],
    managed_close_positions: list[dict[str, Any]],
    pos: Optional[PositionState] = None,
    extra: Optional[dict[str, Any]] = None,
) -> None:
    exchange_values = sorted({str(row.get("Exchange")) for row in managed_close_positions if isinstance(row, dict) and row.get("Exchange") not in (None, "")})
    qty_sum = sum(_to_int(row.get("Qty") if row.get("Qty") is not None else row.get("LeavesQty"), 0) for row in managed_close_positions if isinstance(row, dict))
    payload = {
        "source": source,
        "managed_execution_ids": list(managed_execution_ids or []),
    ...
    storage.log_structured("INFO", "MANAGED_CLOSE_POSITIONS_BUILT", payload)
...
def rebuild_rsi9_long_add_managed_state(
    ...
    log_managed_close_positions_built(
        storage,
        "post_add_rebuild",
        pos.managed_execution_ids,
        pos.managed_close_positions,
        pos,
        {"ts": ts.isoformat(), "raw_positions_json": after_positions},
    )
    ...
    for attempt in range(1, retry_max + 1):
        ...
        if actual is not None:
            ...
            log_managed_close_positions_built(
                storage,
                "entry_verify",
                actual_pos.managed_execution_ids,
                actual_pos.managed_close_positions,
                actual_pos,
                {"ts": ts.isoformat(), "order_id": order_id, "positions_summary": positions_summary},
            )
...
def execute_live_entry(
    ...
    qty_override: Optional[int] = None,
    entry_reference_bar: Optional[Bar] = None,
    market_snapshot_provider: Optional[Callable[[], Optional[TickSnapshot]]] = None,
    ...
    for attempt in range(retries + 1):
        final_pre_send_context: dict[str, Any] = {}
        if (
            is_long_rsi50_trend_hold_only(config)
            and side == "LONG"
            and candidate.strategy == LONG_RSI50_STRATEGY
        ):
            candidate_best_ask = latest_snapshot.sell1_price if latest_snapshot is not None else None
            pre_send_snapshot = market_snapshot_provider() if market_snapshot_provider is not None else None
            if pre_send_snapshot is None:
                raw_board = client.get_board(str(config.get("symbol", SYMBOL_DEFAULT)), int(config.get("exchange", EXCHANGE_DEFAULT)))
                pre_send_snapshot = extract_snapshot(raw_board)
            latest_snapshot = pre_send_snapshot
            reference_close = entry_reference_bar.close if entry_reference_bar is not None else None
            reference_bar_ts = entry_reference_bar.ts if entry_reference_bar is not None else pred.ts
            ...
            final_pre_send_context = {
                ...
                "reference_close": reference_close,
                "candidate_best_ask": candidate_best_ask,
                "final_pre_send_best_ask": pre_send_best_ask,
                "final_pre_send_deviation_ticks": deviation_ticks,
                "entry_reference_close_guard_max_abs_deviation_ticks": max_deviation_ticks,
            }
            if bool(guard_cfg.get("enabled", True)) and (deviation_ticks is None or deviation_ticks > max_deviation_ticks):
                storage.log_structured(
                    "WARN",
                    "LONG_RSI50_TREND_HOLD_ENTRY_BLOCKED_BY_REFERENCE_CLOSE_DEVIATION_FINAL_PRE_SEND",
                    {
                        "reference_bar_ts": reference_bar_ts.isoformat(),
                        "reference_close": reference_close,
                        "candidate_best_ask": candidate_best_ask,
                        ...
                        "max_abs_deviation_ticks": max_deviation_ticks,
                        "rsi9": pred.rsi9_value,
                        "ema13": entry_reference_bar.ema13 if entry_reference_bar is not None else None,
                        "ema13_lookback_value": entry_ema13_lookback_value,
                        "ema13_delta_ticks": entry_ema13_delta_ticks,
                        "block_reason": "FINAL_PRE_SEND_REFERENCE_CLOSE_DEVIATION_TOO_LARGE",
                    },
                    mirror_message=f"best_ask={pre_send_best_ask} reference_close={reference_close} deviation_ticks={deviation_ticks}",
                )
                return LiveOrderResult(False, "FINAL_PRE_SEND_REFERENCE_CLOSE_DEVIATION_TOO_LARGE", recoverable=True)
        limit_price = None
...
def close_positions_qty_sum(close_positions: list[dict[str, Any]]) -> int:
    return sum(_to_int(row.get("Qty"), 0) for row in close_positions if isinstance(row, dict))


def close_position_rows_for_group(pos: PositionState, close_positions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    hold_ids = {str(row.get("HoldID") or row.get("ExecutionID") or "") for row in close_positions if isinstance(row, dict)}
    rows: list[dict[str, Any]] = []
    for row in pos.managed_close_positions or []:
        if not isinstance(row, dict):
            continue
        hold_id = str(row.get("HoldID") or row.get("ExecutionID") or "")
        if hold_id and hold_id in hold_ids:
            rows.append(row)
    return rows
...
def validate_exit_payload(
    ...
    if _to_int(payload.get("CashMargin"), 0) != 3:
        return False, "CASH_MARGIN_NOT_EXIT"
    if _to_int(payload.get("DelivType"), 0) != 2:
        return False, "DELIV_TYPE_NOT_CLOSE"
    ...
    for row in close_positions:
        if not isinstance(row, dict):
            return False, "CLOSE_POSITION_ROW_INVALID"
        if not str(row.get("HoldID") or ""):
            return False, "CLOSE_POSITION_HOLD_ID_MISSING"
        qty = _to_int(row.get("Qty"), 0)
        if qty <= 0:
            return False, "CLOSE_POSITION_QTY_INVALID"
        qty_sum += qty
    if expected_qty > 0 and qty_sum != int(expected_qty):
        return False, "CLOSE_POSITION_QTY_MISMATCH"
    payload_exchange = _to_int(payload.get("Exchange"), 0)
    if payload_exchange <= 0:
        return False, "PAYLOAD_EXCHANGE_MISSING"
    position_exchanges = {
        _to_int(row.get("Exchange"), 0)
        for row in managed_positions
        if isinstance(row, dict) and _to_int(row.get("Exchange"), 0) > 0
    }
    if not position_exchanges:
        return False, "MANAGED_POSITION_EXCHANGE_MISSING"
    if len(position_exchanges) != 1:
        return False, "MANAGED_POSITION_MULTIPLE_EXCHANGES"
    position_exchange = next(iter(position_exchanges))
    if payload_exchange != position_exchange:
        return False, "PAYLOAD_EXCHANGE_MISMATCH"
    expected_exit_side = "1" if pos.side == "LONG" else "2"
    if str(payload.get("Side")) != expected_exit_side:
        return False, "EXIT_SIDE_MISMATCH"
    return True, "OK"


def log_exit_payload_validation_failed(
    storage: Storage,
    context: dict[str, Any],
    reason: str,
    payload: dict[str, Any],
    ...
    storage.log_structured(
        "ERROR",
        "EXIT_PAYLOAD_VALIDATION_FAILED",
        {
            **context,
            "reason": reason,
            "payload": payload,
            "managed_positions": positions_summary_for_log(managed_positions),
            "position_state": position_state_payload(pos),
        },
    )


def reset_exit_pending_after_send_fail(
    storage: Storage,
    status: MonitorStatus,
    pos: PositionState,
    context: dict[str, Any],
    ...
    pending_before = status.pending_exit
    exit_order_id_before = pos.exit_order_id
    status.pending_exit = False
    pos.exit_order_id = None
    storage.log_structured(
        "WARN",
        "EXIT_PENDING_STATE_RESET_AFTER_SEND_FAIL",
        {
            **context,
            "exit_reason": exit_reason,
            "api_code": api_code,
            "pending_exit_before": pending_before,
            "pending_exit_after": status.pending_exit,
            "exit_order_id_before": exit_order_id_before,
            "exit_order_id_after": pos.exit_order_id,
        },
    )
...
def close_position_groups_from_managed_state(
    ...
    """Build ClosePositions from cached bot-managed state only when it is complete and exchange-safe."""
    managed_ids = [str(x) for x in (pos.managed_execution_ids or []) if str(x)]
    if not managed_ids:
        return None, "MANAGED_EXECUTION_IDS_MISSING"
    if not pos.managed_close_positions:
        return None, "EMPTY_MANAGED_CLOSE_POSITIONS"
    ...
    for row in pos.managed_close_positions or []:
        if not isinstance(row, dict):
            continue
        hold_id = str(row.get("HoldID") or row.get("ExecutionID") or "")
        execution_id = str(row.get("ExecutionID") or row.get("HoldID") or "")
        if not hold_id or not execution_id:
            return None, "MISSING_HOLD_ID"
        if execution_id not in managed_ids and hold_id not in managed_ids:
            continue
        seen_ids.add(execution_id if execution_id in managed_ids else hold_id)
        if row.get("Exchange") in (None, ""):
            return None, "MISSING_EXCHANGE"
        row_exchange = _to_int(row.get("Exchange"), 0)
        if row_exchange <= 0:
            return None, "MISSING_EXCHANGE"
        raw_qty = row.get("Qty")
        if raw_qty is None:
        ...
        qty_by_hold_id[hold_id] = max(qty_by_hold_id.get(hold_id, 0), qty)
        exchange_by_hold_id[hold_id] = row_exchange

    missing_ids = [hold_id for hold_id in managed_ids if hold_id not in seen_ids and qty_by_hold_id.get(hold_id, 0) <= 0]
    if missing_ids:
        return None, "MISSING_HOLD_ID"

    exchanges = {exchange_by_hold_id[hold_id] for hold_id in qty_by_hold_id if exchange_by_hold_id.get(hold_id, 0) > 0}
    if not exchanges:
        return None, "MISSING_EXCHANGE"
    if len(exchanges) != 1:
        return None, "MULTIPLE_EXCHANGES"

    position_exchange = next(iter(exchanges))
    close_positions = [{"HoldID": hold_id, "Qty": qty} for hold_id, qty in qty_by_hold_id.items()]
    total_group_qty = close_positions_qty_sum(close_positions)
    if pos.filled_qty > 0 and total_group_qty != int(pos.filled_qty):
        return None, "QTY_MISMATCH"
    if total_group_qty <= 0:
        return None, "NO_MANAGED_CLOSE_QTY"
    return [(position_exchange, close_positions, total_group_qty)], "MANAGED_STATE_WITH_EXCHANGE"
...
def positions_for_close_positions(positions: list[dict[str, Any]], close_positions: list[dict[str, Any]]) -> list[dict[str, Any]]:
    hold_ids = {str(row.get("HoldID") or row.get("ExecutionID") or "") for row in close_positions if isinstance(row, dict)}
    return [p for p in positions if position_execution_id(p) in hold_ids]


def rebuild_exit_groups_after_code8(
    client: KabuApiClient,
    config: dict[str, Any],
    storage: Storage,
    pos: PositionState,
    side: str,
    context: dict[str, Any],
) -> tuple[list[dict[str, Any]], list[tuple[int, list[dict[str, Any]], int]], bool]:
    positions = fetch_positions(client, config, storage, reason="EXIT_CODE8_REBUILD_POSITIONS")
    managed_positions = managed_positions_for(pos, positions) if pos.managed_execution_ids else []
    used_unmanaged_candidate = False
    managed_ids_for_groups: Optional[list[str]] = pos.managed_execution_ids
    if not managed_positions:
        used_unmanaged_candidate = True
        managed_ids_for_groups = None
        managed_positions = [
            p for p in positions
            if position_matches(p, side=side, margin_trade_type=pos.margin_trade_type)
            and position_leaves_qty(p) > 0
            and position_execution_id(p)
        ]
        storage.log_structured(
            "ERROR",
            "EXIT_CODE8_MANAGED_IDS_NOT_FOUND_FALLBACK_TO_SIDE",
            {
                **context,
                "managed_execution_ids": list(pos.managed_execution_ids or []),
                "candidate_positions": positions_summary_for_log(managed_positions),
                "manual_position_mix_risk": True,
            },
        )
    if managed_positions:
        pos.managed_close_positions = positions_summary_for_log(managed_positions)
        pos.managed_execution_ids = sorted({position_execution_id(p) for p in managed_positions if position_execution_id(p)})
        log_managed_close_positions_built(
            storage,
            "positions",
            pos.managed_execution_ids,
            pos.managed_close_positions,
            pos,
            {"reason": "EXIT_CODE8_REBUILD_POSITIONS", "raw_positions_json": positions},
        )
    groups = close_position_groups_for_side(
        positions,
        side,
        margin_trade_type=pos.margin_trade_type,
        available_only=True,
        default_exchange=exit_exchange(config),
        managed_execution_ids=managed_ids_for_groups,
        storage=storage,
    )
    return positions, groups, used_unmanaged_candidate
...
def execute_live_exit(
    ...
    for attempt in range(retries + 1):
        ...
        if fast_path_groups is not None and attempt == 0:
            ...
            if fast_path_groups is not None and attempt > 0:
                storage.log_structured("WARN", "EXIT_FAST_PATH_FALLBACK_TO_POSITIONS_FETCH", {**context, "decision_ts": decision_ts.isoformat(), "attempt": attempt + 1, "fallback_reason": "REPRICE_AFTER_FAST_PATH_TIMEOUT", "position_state": position_state_payload(pos)})
            ...
            if len(close_position_groups) > 1:
                storage.log_structured(
                    "WARN",
                    "EXIT_CLOSE_POSITIONS_SPLIT_BY_EXCHANGE",
                    {
                        **context,
                        "exchanges": [position_exchange for position_exchange, _, _ in close_position_groups],
                        "groups": close_position_groups,
                        "total_qty": total_qty,
                        "order_count": len(close_position_groups),
                    },
                )
            if force_market_order:
            ...
            if managed_positions:
                ...
                log_managed_close_positions_built(
                    ...
                    pos.managed_close_positions,
                    pos,
                    {"reason": "EXIT_BUILD_CLOSE_POSITIONS", "raw_positions_json": positions},
                )
        ...
        is_long_rsi35_exit = pos.strategy == LONG_RSI35_STRATEGY and exit_reason_for_order == LONG_RSI35_EXIT_REASON
        ...
        for position_exchange, close_positions, group_total_qty in close_position_groups:
            if force_market_order:
            ...
            elif is_ma5_trailing_exit or is_scalp_feature_fixed_exit or is_long_rsi50_exit or is_long_rsi35_exit or force_marketable_limit or pos.strategy == "RSI9":
                front_order_type = 20
                ...
                elif is_long_rsi35_exit:
                    exit_order_mode = "best_quote_limit_long_rsi35_exit"
                else:
            ...
            if (is_ma5_trailing_exit or is_scalp_feature_fixed_exit or is_long_rsi50_exit or is_long_rsi35_exit) and not force_market_order:
            ...
            if not valid_payload:
                log_exit_payload_validation_failed(storage, context, validation_reason, payload, validation_positions, pos)
                return LiveOrderResult(False, f"EXIT_PAYLOAD_VALIDATION_FAILED:{validation_reason}", recoverable=True)
            storage.log_structured(
                "INFO",
                "EXIT_PAYLOAD_VALIDATED",
                {
                    **context,
                    "attempt": attempt + 1,
            ...
            storage.log_structured(
                "INFO",
                "EXIT_ORDER_BUILD_FROM_MANAGED_POSITIONS",
                {
                    **context,
                    "attempt": attempt + 1,
                    "position_exchange": position_exchange,
                    "close_positions": close_positions,
                    "managed_positions": positions_summary_for_log(validation_positions),
                    "request_json": payload,
                    "fast_path": not bool(positions),
                },
            )
            storage.log_structured(
                "INFO",
            ...
            try:
                ...
                reset_exit_pending_after_send_fail(storage, status, pos, context, str(exit_reason_for_order), code)
                if code == "8":
                    storage.log_structured(
                        "ERROR",
                        "EXIT_ORDER_SEND_FAIL_CODE8_REBUILD",
                        {
                            **context,
                            "original_payload": payload,
                            "original_exchange": position_exchange,
                            "original_close_positions": close_positions,
                            "api_error": ep,
                        },
                    )
                    if exit_reason_for_order == "HARD_STOP_LOSS":
                        storage.log_structured(
                            "ERROR",
                            "HARD_STOP_CODE8_EMERGENCY_REBUILD",
                            {
                                **context,
                                "entry_price": pos.entry_price,
                                "current_price": signal_price,
                                "pnl_ticks": pnl_ticks,
                                "original_exit_reason": "HARD_STOP_LOSS",
                                "original_exchange": position_exchange,
                                "retry_result": "START_REBUILD",
                            },
                        )
                    try:
                        rebuilt_positions, rebuilt_groups, used_unmanaged_candidate = rebuild_exit_groups_after_code8(client, config, storage, pos, side, context)
                    except Exception as rebuild_exc:
                        rebuild_ep = api_error_payload(rebuild_exc)
                        storage.log_structured(
                            "ERROR",
                            "EXIT_ORDER_SEND_FAIL_CODE8_REBUILD",
                            {
                                **context,
                                ...
                                "original_close_positions": close_positions,
                                "api_error": ep,
                                "rebuild_error": rebuild_ep,
                            },
                        )
                        return LiveOrderResult(False, f"EXIT_CODE8_REBUILD_POSITIONS_FAILED: {rebuild_ep.get('raw_error')}", api_code=code, api_message=str(ep.get("api_message") or ""), recoverable=True)
                        ...
                        storage.log_structured(
                            ...
                            "EXIT_ORDER_SEND_FAIL_CODE8_REBUILD",
                            {
                                **context,
                                ...
                                "original_close_positions": close_positions,
                                "api_error": ep,
                                "rebuilt_positions": positions_summary_for_log(rebuilt_positions),
                                "rebuilt_close_positions": [],
                                "retry_result": "NO_REBUILT_GROUPS",
                            },
                        )
                        return LiveOrderResult(False, "EXIT_CODE8_REBUILD_NO_CLOSE_POSITIONS", api_code=code, api_message=str(ep.get("api_message") or ""), recoverable=True)
                    if len(rebuilt_groups) > 1:
                        storage.log_structured(
                            "WARN",
                            "EXIT_CLOSE_POSITIONS_SPLIT_BY_EXCHANGE",
                            {
                                **context,
                                "exchanges": [rebuilt_exchange for rebuilt_exchange, _, _ in rebuilt_groups],
                                "groups": rebuilt_groups,
                                "total_qty": sum(rebuilt_qty for _, _, rebuilt_qty in rebuilt_groups),
                                "order_count": len(rebuilt_groups),
                                "source": "code8_rebuild",
                            },
                        )
                    retry_failed = False
                    for rebuilt_exchange, rebuilt_close_positions, rebuilt_group_total_qty in rebuilt_groups:
                        retry_payload = build_exit_order_payload(
                            config,
                            side,
                            close_positions=rebuilt_close_positions,
                            qty=rebuilt_group_total_qty,
                            front_order_type=front_order_type,
                            price=limit_price,
                            margin_trade_type=pos.margin_trade_type,
                            exchange=rebuilt_exchange,
                        )
                        retry_positions_for_group = positions_for_close_positions(rebuilt_positions, rebuilt_close_positions)
                        retry_valid, retry_validation_reason = validate_exit_payload(retry_payload, retry_positions_for_group, pos, rebuilt_group_total_qty)
                        if not retry_valid:
                            log_exit_payload_validation_failed(storage, context, retry_validation_reason, retry_payload, retry_positions_for_group, pos)
                            retry_failed = True
                            last = LiveOrderResult(False, f"EXIT_CODE8_RETRY_PAYLOAD_INVALID:{retry_validation_reason}", api_code=code, api_message=str(ep.get("api_message") or ""), recoverable=True)
                            break
                        storage.log_structured(
                            "INFO",
                            "EXIT_PAYLOAD_VALIDATED",
                            {
                                **context,
                                "position_exchange": rebuilt_exchange,
                                "close_positions": rebuilt_close_positions,
                                "group_total_qty": rebuilt_group_total_qty,
                                "payload_exchange": retry_payload.get("Exchange"),
                        ...
                        storage.log_structured(
                            ...
                            "EXIT_ORDER_SEND_FAIL_CODE8_REBUILD",
                            {
                                **context,
                                ...
                                "api_error": ep,
                                "rebuilt_positions": positions_summary_for_log(rebuilt_positions),
                                "rebuilt_exchange": rebuilt_exchange,
                                "rebuilt_close_positions": rebuilt_close_positions,
                                "retry_payload": retry_payload,
                                "used_unmanaged_candidate": used_unmanaged_candidate,
                            },
                        )
                        try:
                            retry_send_ts = now_jst()
                            retry_res = client.send_order(retry_payload)
                            retry_response_ts = now_jst()
                        except Exception as retry_exc:
                            retry_ep = api_error_payload(retry_exc)
                            reset_exit_pending_after_send_fail(storage, status, pos, context, str(exit_reason_for_order), str(retry_ep.get("api_code") or ""))
                            storage.log_structured(
                                "ERROR",
                                "EXIT_ORDER_SEND_FAIL_CODE8_REBUILD",
                                {
                                    **context,
                                    ...
                                    "rebuilt_close_positions": rebuilt_close_positions,
                                    "retry_payload": retry_payload,
                                    "retry_error": retry_ep,
                                },
                            )
                            retry_failed = True
                            last = LiveOrderResult(False, f"EXIT_CODE8_RETRY_SEND_ERROR Code={retry_ep.get('api_code')} Message={retry_ep.get('api_message')}: {retry_ep.get('raw_error')}", api_code=str(retry_ep.get("api_code") or code), api_message=str(retry_ep.get("api_message") or ""), recoverable=True)
                            break
                        retry_order_id = str(retry_res.get("OrderId") or retry_res.get("OrderID") or "")
                        storage.log_structured(
                            "INFO",
                            "EXIT_ORDER_SEND_SUCCESS_AFTER_CODE8_REBUILD",
                            {
                                **context,
                        ...
                        storage.log_structured(
                            ...
                            "EXIT_ORDER_SEND_TIMING",
                            {
                                **context,
                                "decision_ts": decision_ts.isoformat(),
                                "order_send_ts": retry_send_ts.isoformat(),
                                "order_response_ts": retry_response_ts.isoformat(),
                                "decision_to_send_ms": (retry_send_ts - decision_ts).total_seconds() * 1000.0,
                                "send_to_response_ms": (retry_response_ts - retry_send_ts).total_seconds() * 1000.0,
                                "decision_to_response_ms": (retry_response_ts - decision_ts).total_seconds() * 1000.0,
                                "order_id": retry_order_id,
                                "side": side,
                                "exit_reason": exit_reason_for_order,
                                "signal_price": signal_price,
                        ...
                        if not retry_order_id:
                            retry_failed = True
                            last = LiveOrderResult(False, f"EXIT_CODE8_RETRY_ORDER_ID_MISSING: {retry_res}", recoverable=True)
                            break
                        order_ids.append(retry_order_id)
                        last = LiveOrderResult(False, "EXIT_ORDER_SENT_AFTER_CODE8_REBUILD", order_id=retry_order_id, limit_price=limit_price, order_send_ts=retry_send_ts, order_response_ts=retry_response_ts, recoverable=True)
                    if retry_failed:
                        return last
                    break
                if code != "8":
...
def force_close_open_position(
    ...
    diag_cfg = config.get("diagnostic_logging", {}) if isinstance(config.get("diagnostic_logging"), dict) else {}
    if not bool(diag_cfg.get("force_close_start_log_once", True)) or not status.force_market_close_start_logged:
        ...
        status.force_market_close_start_logged = True
...
def reconcile_live_position(
    ...
    if total_qty == 0:
        if status.open_position is not None:
            ...
            status.last_reconcile_ok_ts = ts
            status.last_reconcile_live_state = status.live_state
            status.last_reconcile_open_orders_count = 0
            status.last_reconcile_positions_count = len(positions)
            status.last_reconcile_verified_flat = True
...
def run_monitor(config: dict[str, Any]) -> tuple[str, str]:
    ...
    def sleep_or_wait_for_market_data() -> None:
        sleep_sec = float(config.get("poll_interval_sec", POLL_INTERVAL_SEC))
        if market_data_mode == "websocket" and ws_feed is not None:
            sleep_sec = min(sleep_sec, max(websocket_loop_sleep_sec, 0.01))
            ws_feed.wait_for_data(sleep_sec)
        else:
    ...
    while True:
        ...
        try:
            ...
            snapshots_to_process: list[TickSnapshot] = []
            final_snapshot_source = "rest"
            ...
            if market_data_mode == "websocket" and ws_feed is not None:
                ...
                if drained:
                    ...
                    raw_snapshot_persistence_enabled = persist_raw_ws_snapshots
                    ws_metrics = ws_feed.metrics()
                    last_processed_after_preview = drained[-1].ts if drained else last_processed_snapshot_ts
                    oldest_age_ms = max((now_ - drained[0].ts).total_seconds() * 1000.0, 0.0)
                    newest_age_ms = max((now_ - drained[-1].ts).total_seconds() * 1000.0, 0.0)
                    diag_cfg = config.get("diagnostic_logging", {}) if isinstance(config.get("diagnostic_logging"), dict) else {}
                    drain_payload = {
                        "drained_count": len(drained),
                        "queue_len_after": ws_metrics.get("queue_len"),
                        "received_count_total": ws_metrics.get("received_count_total"),
                        "drained_count_total": ws_metrics.get("drained_count_total"),
                        "dropped_count_total": ws_metrics.get("dropped_count_total"),
                        "first_snapshot_ts": drained[0].ts.isoformat(),
                        "last_snapshot_ts": drained[-1].ts.isoformat(),
                        "last_processed_snapshot_ts_before": drained_before_ts.isoformat() if drained_before_ts is not None else None,
                        "last_processed_snapshot_ts_after": last_processed_after_preview.isoformat(),
                        "oldest_snapshot_age_ms": oldest_age_ms,
                        "newest_snapshot_age_ms": newest_age_ms,
                    }
                    if bool(diag_cfg.get("ws_queue_drain_log_each_drain", False)):
                        storage.log_structured("INFO", "WS_QUEUE_DRAINED", drain_payload)
                    record_ws_queue_drain_summary(status, storage, config, now_, drained, ws_metrics, oldest_age_ms, newest_age_ms)
                    if int(ws_metrics.get("dropped_count_total") or 0) > 0:
                        storage.log_structured(
                            "WARN",
                            "WS_QUEUE_OVERFLOW_WARN",
                            {
                                "queue_maxlen": ws_metrics.get("queue_maxlen"),
                                "received_count_total": ws_metrics.get("received_count_total"),
                                "dropped_count_total": ws_metrics.get("dropped_count_total"),
                    ...
                    if not should_rest_fallback:
                        ...
                        storage.log_structured(
                            ...
                            "REST_SNAPSHOT_USED",
                            {
                                "snapshot_ts": rest_snap.ts.isoformat(),
                                "now_ts": now_.isoformat(),
                                "age_sec": (now_ - rest_snap.ts).total_seconds(),
                                "last_processed_snapshot_ts": last_processed_snapshot_ts.isoformat() if last_processed_snapshot_ts is not None else None,
                                "fallback_to_rest": True,
                                "reason": "WS_QUEUE_EMPTY_FALLBACK",
                            },
                        )
            if market_data_mode != "websocket" or ws_feed is None:
                raw = client.get_board(config["symbol"], config["exchange"])
                rest_snap = extract_snapshot(raw)
                snapshots_to_process = [rest_snap]
                final_snapshot_source = "rest"
            ...
            bar1_queue_finalized: Optional[Bar] = None
            ...
            for snap_candidate in snapshots_to_process:
                ...
                spread_ticks = calc_spread_ticks(snap_candidate)
                should_persist_raw = raw_snapshot_persistence_enabled
                if final_snapshot_source == "websocket_queue" and persist_raw_ws_snapshot_every_n > 0:
                    should_persist_raw = accepted_count % persist_raw_ws_snapshot_every_n == 0
                if should_persist_raw:
                    storage.insert_snapshot(snap_candidate, spread_ticks)
                status.count += 1
                updated_1m = rb1.update(snap_candidate, bar_finalize_delay_ms)
                if updated_1m is not None:
                    ...
                    if final_snapshot_source == "websocket_queue":
                        storage.log_structured(
                            "INFO",
                            "WS_BAR_FINALIZED_FROM_QUEUE",
                            {
                                "bar_ts": updated_1m.ts.isoformat(),
                                "finalize_ts": now_jst().isoformat(),
                                "source": final_snapshot_source,
                                "open": updated_1m.open,
                                "high": updated_1m.high,
                                "low": updated_1m.low,
                                "close": updated_1m.close,
                ...
                updated_3m = rb3.update(snap_candidate, bar_finalize_delay_ms)
                if updated_3m is not None:
                    bar3_queue_finalized = updated_3m
                    storage.insert_bar("bars_3m", updated_3m)
            ...
            if force_close_time_reached_top:
                diag_cfg = config.get("diagnostic_logging", {}) if isinstance(config.get("diagnostic_logging"), dict) else {}
                if status.open_position is not None and (not bool(diag_cfg.get("force_close_start_log_once", True)) or not status.force_market_close_start_logged):
            ...
            bar3_new = bar3_time_finalized or bar3_queue_finalized
            if bar3_time_finalized:
                storage.insert_bar("bars_3m", bar3_time_finalized)
            ...
            if f is not None:
                ...
                long_rsi50_and_rsi35 = is_long_rsi50_and_rsi35_mode(config)
                long_rsi_managed = is_long_rsi_managed_mode(config)
                prev_rsi35_for_decision = status.rsi35_rebound_last_rsi9
                prev_rsi50_for_decision = status.rsi50_cycle_last_rsi9
                if long_rsi_managed and status.pending_add:
                    status.pending_add = False
                    storage.log("INFO", "LONG_RSI_MANAGED_MODE_PENDING_ADD_CLEARED", "reason=ADD_DISABLED_IN_LONG_RSI_MANAGED_MODE")
                if long_rsi_managed and status.pending_entry_side not in {None, "LONG"}:
                    ...
                    storage.log_structured(
                        ...
                        "LONG_RSI50_MODE_SHORT_PENDING_ENTRY_CLEARED",
                        {
                            ...
                            "reason": "SHORT_ENTRY_FORBIDDEN_IN_LONG_RSI_MANAGED_MODE",
                        },
                    )
                if long_rsi_managed:
                    p = None
                    if long_rsi50_and_rsi35:
                        p35 = build_long_rsi35_rebound_prediction(
                            rb1.latest(),
                            list(rb1.history),
                            ...
                            prev_rsi9_for_decision=prev_rsi35_for_decision,
                        )
                        if p35 is not None and p35.signal == "LONG_CANDIDATE":
                            p = p35
                    if p is None:
                        p = build_long_rsi50_trend_hold_prediction(
                            ...
                            allow_new_entry=allow_new_entry,
                            new_entry_cutoff_reached=entry_cutoff_reached,
                            prev_rsi9_for_decision=prev_rsi50_for_decision,
                        )
                else:
                ...
                feature_cfg = config.get("feature_entries", {}) if isinstance(config.get("feature_entries", {}), dict) else {}
                feature_enabled = bool(feature_cfg.get("enabled", False))
                if bar1_new is not None and not long_rsi_managed:
                ...
                feature_candidates: list[PredictionSnapshot] = [] if long_rsi_managed else build_scalp_feature_candidates(
                    f,
                    metrics,
                    config,
                    p.rsi9_value,
                ...
                if (not long_rsi_managed) and not entry_cutoff_reached and status.open_position is None and allow_new_entry:
                ...
                if (not long_rsi_managed) and feature_candidates:
                ...
                if (not long_rsi_managed) and log_big_trend_score and bar1_new is not None:
                ...
                if bool(volatility_gate.logging.get("save_gate_decision", True)) and should_save_gate_decision(status, config, p, gate_decision):
                ...
                if (not force_close_handled_above) and bar1_new is not None and status.pending_entry_side is None and status.open_position is None:
                    if effective_signal in {"LONG_CANDIDATE", "SHORT_CANDIDATE"}:
                        ...
                        if long_rsi_managed and (pending_side != "LONG" or p.reason_3 not in {LONG_RSI50_ENTRY_RULE, LONG_RSI35_ENTRY_RULE}):
                            storage.log_structured(
                                "ERROR",
                ...
                if (not long_rsi_managed) and (not force_close_handled_above) and bar1_new is not None and status.open_position is not None and status.live_state == "OPEN":
                ...
                if (not long_rsi_managed) and status.pending_add and status.open_position is not None and status.open_position.side == "LONG":
                ...
                if status.pending_entry_side and status.open_position is None:
                    if force_close_handled_above:
                        ...
                        side = status.pending_entry_side
                        if long_rsi_managed and side != "LONG":
                            storage.log_structured(
                                "ERROR",
                        ...
                        if enter_ok:
                            ...
                            entry_trigger_ts = status.pending_entry_ts or p.ts
                            position_reconcile_started_ts = None
                            position_reconcile_finished_ts = None
                            pre_send_snapshot_started_ts = None
                            pre_send_snapshot_finished_ts = None
                            used_reconcile_fast_path = False
                            if candidate_pos.side != side:
                                ...
                                fast_ok, fast_reason, fast_age_sec = can_use_position_reconcile_fast_path(status, config, f.ts)
                                if fast_ok:
                                    used_reconcile_fast_path = True
                                    rec = ReconcileResult(True, status.live_state, "FAST_PATH_VERIFIED_FLAT", total_leaves_qty=0, matching_leaves_qty=0, positions=[])
                                    storage.log_structured("INFO", "POSITION_RECONCILE_FAST_PATH_USED", {
                                        "strategy": candidate_pos.strategy, "entry_rule": candidate_pos.entry_rule, "bar_ts": p.ts.isoformat(),
                                        "last_reconcile_ok_ts": status.last_reconcile_ok_ts.isoformat() if status.last_reconcile_ok_ts else None,
                                        "age_sec": fast_age_sec,
                                        "last_reconcile_live_state": status.last_reconcile_live_state,
                                        "last_reconcile_open_orders_count": status.last_reconcile_open_orders_count,
                                        "last_reconcile_positions_count": status.last_reconcile_positions_count,
                                        "last_reconcile_verified_flat": status.last_reconcile_verified_flat,
                                    })
                                else:
                                    log_structured_once_per_bar(storage, status, "INFO", "POSITION_RECONCILE_FAST_PATH_SKIPPED", {"reason": fast_reason, "strategy": candidate_pos.strategy, "entry_rule": candidate_pos.entry_rule, "bar_ts": p.ts.isoformat(), "live_state": status.live_state, "pending_entry": status.pending_entry_side, "pending_exit": status.pending_exit, "pending_add": status.pending_add, "last_reconcile_ok_ts": status.last_reconcile_ok_ts.isoformat() if status.last_reconcile_ok_ts else None}, p.ts, dedupe_extra=fast_reason)
                                    position_reconcile_started_ts = now_jst()
                                    ...
                                    position_reconcile_finished_ts = now_jst()
                                if not rec.ok_for_entry or status.open_position is not None:
                                    ...
                                    if status.last_entry_reject_key and status.last_entry_reject_key == reject_key:
                                        ...
                                        if long_rsi_managed and candidate_pos.strategy in {LONG_RSI50_STRATEGY, LONG_RSI35_STRATEGY}:
                                            pre_send_snapshot_started_ts = now_jst()
                                            ...
                                            if market_data_mode == "websocket" and ws_feed is not None:
                                                ...
                                                if usable_pre_send_ws:
                                                    ...
                                                    pre_send_snapshot_finished_ts = now_jst()
                                                    pre_send_event = "LONG_RSI35_REBOUND_ENTRY_BLOCKED_BY_REFERENCE_CLOSE_DEVIATION_PRE_SEND" if candidate_pos.strategy == LONG_RSI35_STRATEGY else "LONG_RSI50_TREND_HOLD_ENTRY_BLOCKED_BY_REFERENCE_CLOSE_DEVIATION_PRE_SEND"
                                                    storage.log_structured(
                                                        "WARN",
                                                        pre_send_event,
                                                        {
                                                            ...
                                                            "entry_deviation_ticks": None,
                                                            "max_abs_deviation_ticks": (
                                                                long_rsi35_rebound_config(config).get("entry_reference_close_guard_max_abs_deviation_ticks", 4)
                                                                if candidate_pos.strategy == LONG_RSI35_STRATEGY
                                                                else entry_reference_close_guard_config(config).get("max_abs_deviation_ticks", 4)
                                                            ),
                                                            "rsi9": p.rsi9_value,
                                                            ...
                                                            "trigger_ts": entry_trigger_ts.isoformat(),
                                                            "pre_send_check_ts": pre_send_snapshot_finished_ts.isoformat(),
                                                            "elapsed_ms_from_trigger": (pre_send_snapshot_finished_ts - entry_trigger_ts).total_seconds() * 1000.0,
                                                            "market_data_reason": pre_send_reason,
                                                        },
                                                    )
                                                    log_entry_signal_to_order_latency(storage, config, candidate_pos, side, p, entry_trigger_ts, None, position_reconcile_started_ts, position_reconcile_finished_ts, pre_send_snapshot_started_ts, pre_send_snapshot_finished_ts, rb1.latest().close if rb1.latest() is not None else None, None, None, None, "PRE_SEND_MARKET_DATA_UNAVAILABLE", used_reconcile_fast_path, status)
                                                    status.pending_entry_side = None
                                                    status.pending_entry_ts = None
                                                    status.live_state = "FLAT"
                                                    continue
                                            ...
                                            deviation_ticks = reference_close_deviation_ticks(pre_send_best_ask, reference_close)
                                            if candidate_pos.strategy == LONG_RSI35_STRATEGY:
                                                rsi35_guard_cfg = long_rsi35_rebound_config(config)
                                                max_deviation_ticks = float(rsi35_guard_cfg.get("entry_reference_close_guard_max_abs_deviation_ticks", 4))
                                                guard_enabled = bool(rsi35_guard_cfg.get("entry_reference_close_guard_enabled", True))
                                            else:
                                                guard_cfg = entry_reference_close_guard_config(config)
                                                max_deviation_ticks = float(guard_cfg.get("max_abs_deviation_ticks", 4))
                                            ...
                                            if guard_enabled and (deviation_ticks is None or deviation_ticks > max_deviation_ticks):
                                                ...
                                                storage.log_structured(
                                                    ...
                                                    pre_send_event,
                                                    {
                                                        "strategy": candidate_pos.strategy,
                                                        "entry_rule": candidate_pos.entry_rule,
                                                        "bar_ts": reference_bar.ts.isoformat() if reference_bar is not None else p.ts.isoformat(),
                                                        "reference_bar_ts": reference_bar.ts.isoformat() if reference_bar is not None else p.ts.isoformat(),
                                                        "reference_close": reference_close,
                                                        "best_ask": pre_send_best_ask,
                                                        "best_bid": entry_send_snapshot.buy1_price if entry_send_snapshot is not None else None,
                                                        "pre_signal_best_ask": entry_send_snapshot.sell1_price if entry_send_snapshot is not None else None,
                                                        "pre_send_best_ask": pre_send_best_ask,
                                                        "entry_deviation_ticks": deviation_ticks,
                                                        "max_abs_deviation_ticks": max_deviation_ticks,
                                                ...
                                                log_entry_signal_to_order_latency(storage, config, candidate_pos, side, p, entry_trigger_ts, None, position_reconcile_started_ts, position_reconcile_finished_ts, pre_send_snapshot_started_ts, pre_send_snapshot_finished_ts, reference_close, pre_send_best_ask, entry_send_snapshot.buy1_price if entry_send_snapshot is not None else None, deviation_ticks, "PRE_SEND_REFERENCE_CLOSE_DEVIATION_TOO_LARGE", used_reconcile_fast_path, status)
                                                status.pending_entry_side = None
                                                status.pending_entry_ts = None
                                                status.live_state = "FLAT"
                                                continue
                                        reference_bar_for_entry = rb1.latest()
                                        lookback_bars_for_entry = int(long_rsi50_trend_hold_config(config).get("ema13_lookback_bars", 4))
                                        ema13_lookback_for_entry_bar = rb1.prev(lookback_bars_for_entry)
                                        ema13_lookback_for_entry = ema13_lookback_for_entry_bar.ema13 if ema13_lookback_for_entry_bar is not None else None
                                        ema13_delta_for_entry_ticks = (
                                            price_to_ticks(reference_bar_for_entry.ema13 - ema13_lookback_for_entry, reference_bar_for_entry.close)
                                            if reference_bar_for_entry is not None
                                            and reference_bar_for_entry.ema13 is not None
                                            and ema13_lookback_for_entry is not None
                                            else None
                                        )

                                        def final_pre_send_ws_snapshot_provider() -> Optional[TickSnapshot]:
                                            if market_data_mode != "websocket" or ws_feed is None:
                                                return None
                                            ws_pre_send = ws_feed.latest_snapshot()
                                            usable_pre_send_ws, _reason, _age = websocket_snapshot_freshness(
                                                ws_pre_send,
                                                ws_feed.available,
                                                now_jst(),
                                                max_ws_snapshot_age_sec,
                                            ...
                                            return ws_pre_send if usable_pre_send_ws else None

                                        order_request_ts_for_latency = now_jst()
                                        log_entry_signal_to_order_latency(
                                            storage,
                                            config,
                                            candidate_pos,
                                            ...
                                            p,
                                            entry_trigger_ts,
                                            order_request_ts_for_latency,
                                            position_reconcile_started_ts,
                                            position_reconcile_finished_ts,
                                            pre_send_snapshot_started_ts,
                                            pre_send_snapshot_finished_ts,
                                            reference_bar_for_entry.close if reference_bar_for_entry is not None else None,
                                            entry_send_snapshot.sell1_price if entry_send_snapshot is not None else None,
                                            entry_send_snapshot.buy1_price if entry_send_snapshot is not None else None,
                                            reference_close_deviation_ticks(entry_send_snapshot.sell1_price if entry_send_snapshot is not None else None, reference_bar_for_entry.close if reference_bar_for_entry is not None else None),
                                            "",
                                            used_reconcile_fast_path,
                                            status,
                                        )
                                        result = execute_live_entry(
                                            ...
                                            p,
                                            status,
                                            latest_snapshot=entry_send_snapshot,
                                            entry_reference_bar=reference_bar_for_entry,
                                            market_snapshot_provider=final_pre_send_ws_snapshot_provider,
                                            entry_ema13_lookback_value=ema13_lookback_for_entry,
                                            entry_ema13_delta_ticks=ema13_delta_for_entry_ticks,
                                        )
                        ...
                        if ex:
                            ...
                            if config["live_mode"]:
                                ...
                                if exit_confirmed and live_tp_already_filled:
                                    ...
                                    status.live_state = "FLAT"
                                    if pos.strategy == LONG_RSI35_STRATEGY:
                                        reset_rsi35_rebound_cycle(status, storage, f.ts, "EXIT_FILLED", p.rsi9_value, bar1_new.ts if bar1_new is not None else None)
                                    status.open_position = None
                                    status.pending_exit = False
                                    status.pending_add = False
                                    ...
                                    result = execute_live_exit(
                                        ...
                                        status,
                                        latest_snapshot=decision_snapshot or last_snapshot,
                                        force_marketable_limit=(
                                            pos.strategy == "RSI9"
                                            or ex_reason in MA5_TRAILING_EXIT_REASONS
                                            or (pos.strategy == LONG_RSI50_STRATEGY and ex_reason == LONG_RSI50_EXIT_REASON)
                                            or (pos.strategy == LONG_RSI35_STRATEGY and ex_reason == LONG_RSI35_EXIT_REASON)
                                        ),
                ...
                if long_rsi_managed and p.rsi9_value is not None:
                    status.rsi35_rebound_last_rsi9 = p.rsi9_value
                    status.rsi50_cycle_last_rsi9 = p.rsi9_value
